...
- Use `#SBATCH --gres=gpu:L4:1` for every Blender render, preview, benchmark, and environment-probe job in this project.
- Do not request L40S GPUs for future jobs. Scale with L4 array tasks when more throughput is needed.
- Historical reports may retain the GPU model actually used for completed renders.
