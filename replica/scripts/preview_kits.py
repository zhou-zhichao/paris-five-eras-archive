...
cam.data.type = 'ORTHO'; cam.data.ortho_scale = 560; cam.data.clip_end = 5000
cam.location = (250, -230 - 300, 400)
cam.rotation_euler = ((cam.location - Vector((250, -230, 0))).to_track_quat('Z', 'Y').to_euler())
ground = bpy.data.objects.new("G", bpy.data.meshes.new("G")); scene.collection.objects.link(ground)
import bmesh
bm = bmesh.new(); bmesh.ops.create_grid(bm, x_segments=1, y_segments=1, size=600); bm.to_mesh(ground.data); bm.free()
ground.location = (250, -230, -0.05)
